package EPrints::Plugin::Stats::View::ReportHeaderZORA;
...
sub render_breadcrumbs
{
	...
	{
		my $report = $context->{irs2report} || '';
		my $context_args = "?range=$context->{range}&from=$context->{from}&to=$context->{to}";

		my $url = "/cgi/stats/report/$report" . $context_args;
		...
		{
			$bd->appendChild( $session->make_text( " (" ) );
			my $eprint_url = "/$set->{set_value}/";
			my $eprint_link = $bd->appendChild( $session->make_element( 'a', href => $eprint_url ) );
			$eprint_link->appendChild( $session->html_phrase( "lib/irstats2/link:eprint" ) );
			$bd->appendChild( $session->make_text( ")" ) );
		}
	}
	else
...
sub render_timeline
{
	...
	{
        my $url = $session->config( 'http_cgiurl' ).'/stats/report/';
	
		if( defined $context->{set_name} && defined $context->{set_value} )
		{
			$url .= "$context->{set_name}/$context->{set_value}/$context->{irs2report}";
		}
		else
		{
			$url .= "$context->{irs2report}";
		}
	
		my $from = $context->get_property( 'from' );
		my $to = $context->get_property( 'to' );

		$tl->appendChild( EPrints::Plugin::Stats::Utils::render_date( $session, { from => $from, to => $to } ) );
		$tl->appendChild( $session->make_text( " < " ) );
		
		my $level1 = $tl->appendChild( $session->make_element( 'a', href => $url ) );
		$level1->appendChild( $session->html_phrase( "lib/irstats2/timeline:all" ) );
	}
...
sub render_filters
{
	...
	my $reports_div = $frag->appendChild( $session->make_element( 'div', id => 'irstats2_reports', class => 'irstats2_view' ) );
	my $reports_box = $reports_div->appendChild( $session->make_element( 'div', class => 'irstats2_view_title' ) );
	
	$reports_box->appendChild( $session->html_phrase( 'lib/irstats2/header:available_reports' ) );
    ...
    my( $table, $tr, $td );
# ToDo: table2ul-horizontal
    $table = $reports_div->appendChild( $session->make_element( 'table', border => '0', cellpadding => '0', cellspacing => '0', class => 'irstats2_view_content' ) );
    ...
    foreach my $category ( keys %$reports )
    {
		...
		{
			my $span = $td->appendChild( $session->make_element( 'span', style => 'margin: 10px;' ) );
			# a bit hacky
...
#   ZORA-507(15) table2div - 2016/10/24/jv
    my $filters_dates;
    $filters_dates = $frag->appendChild( $session->make_element( 'div', id=> 'irstats2_filters_dates', class => 'irstats2_view_Grid' ) );
    ...
    my $filters_div = $filters_dates->appendChild( $session->make_element( 'div', id => 'irstats2_filters', class => 'irstats2_view' ) );
	
    my $div = $filters_div->appendChild( $session->make_element( 'div', class => 'irstats2_view_title' ) );
	$div->appendChild( $session->html_phrase( 'lib/irstats2/header:filter_items' ) );

	my $filters = $filters_div->appendChild( $session->make_element( 'div', id => 'irstats2_filters' ) );
    ...
    $filters->appendChild( $session->make_element( 'input', type => "text", name => "setdesc_q", id => "setdesc_q", size => '25', class => 'irstats2_setdesc_q' ) );
...
JS
    ...
    my $dates_div = $filters_dates->appendChild( $session->make_element( 'div', id => 'irstats2_dates', align => 'center', class => 'irstats2_view' ) );
    $div = $dates_div->appendChild( $session->make_element( 'div', class => 'irstats2_view_title' ) );
    $div->appendChild( $session->html_phrase( 'lib/irstats2/header:dates' ) );
	...
	my $dates = $dates_div->appendChild( $session->make_element( 'div' ) );
...
#	ZORA-507(15) table2div - 2016/10/24/jv
	...
	my $breadcrumbs = $content->appendChild( $session->make_element( 'div', class => 'irstats2_ReportHeader_breadcrumbs' ) );
	$breadcrumbs->appendChild( $self->render_breadcrumbs );
	
	my $timeline = $content->appendChild( $session->make_element( 'div', class => 'irstats2_ReportHeader_timeline' ) );
#	ZORA-507(15) table2div - 2016/10/24/jv

	$timeline->appendChild( $self->render_timeline );
	
	my $options = $frag->appendChild( $self->render_filters );
